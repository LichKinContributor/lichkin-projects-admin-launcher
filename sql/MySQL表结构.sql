...
CREATE TABLE IF NOT EXISTS `t_sys_comp` (
  ...
  `ABBREVIATION` varchar(6) DEFAULT NULL COMMENT '简名（用于客户端显示）',
...
CREATE TABLE IF NOT EXISTS `t_sys_calendar` (
  `ID` varchar(64) NOT NULL COMMENT '主键',
  `CALENDAR_DATE` varchar(10) NOT NULL COMMENT '日期（yyyy-MM-dd）',
  `YEAR` smallint(6) NOT NULL COMMENT '年份',
  `MONTH` tinyint(4) NOT NULL COMMENT '月份',
  `QUARTER` tinyint(4) NOT NULL COMMENT '季度',
  `WEEK` tinyint(4) NOT NULL COMMENT '一年中第几周',
  `DAY_OF_MONTH` tinyint(4) NOT NULL COMMENT '一月中第几天',
  `DAY_OF_WEEK` tinyint(4) NOT NULL COMMENT '一周中第几天',
  `HOLIDAY` bit(1) NOT NULL COMMENT '是否为法定节假日',
  PRIMARY KEY (`ID`)
...
CREATE TABLE IF NOT EXISTS `t_sys_holiday` (
  `ID` varchar(64) NOT NULL COMMENT '主键',
  `CALENDAR_DATE` varchar(10) NOT NULL COMMENT '日期（yyyy-MM-dd）',
  `HOLIDAY_TYPE` varchar(1) NOT NULL COMMENT '法定节假日类型',
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='法定节假日表';

-- DROP TABLE IF EXISTS `t_sys_hour`;
CREATE TABLE IF NOT EXISTS `t_sys_hour` (
  `ID` varchar(64) NOT NULL COMMENT '主键',
  `START_MINUTES` smallint(6) NOT NULL COMMENT '开始分钟',
  `END_MINUTES` smallint(6) NOT NULL COMMENT '结束分钟',
  `HOUR_TEXT1` varchar(255) NOT NULL COMMENT '小时显示值',
  `HOUR_TEXT2` varchar(255) NOT NULL COMMENT '小时显示值',
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='小时表';
...
CREATE TABLE IF NOT EXISTS `t_sys_activiti_api_request_log_start_process` (
  ...
  `APP_KEY` varchar(128) DEFAULT NULL COMMENT '客户端唯一标识（字典）',
...
-- DROP TABLE IF EXISTS `t_sys_app_employee_module_config`;
CREATE TABLE IF NOT EXISTS `t_sys_app_employee_module_config` (
  `ID` varchar(64) NOT NULL COMMENT '主键',
  `MODULE_TYPE` varchar(64) NOT NULL COMMENT '模块类型（字典）',
  `MODULE_NAME` varchar(64) NOT NULL COMMENT '模块名称',
  `ON_LINE` bit(1) NOT NULL COMMENT 'true:已上线;false:未上线;',
  `AUTH` bit(1) NOT NULL COMMENT 'true:需要权限;false:不需要权限;',
  `ICON` varchar(32) DEFAULT NULL COMMENT '图标',
  `URL` varchar(128) NOT NULL COMMENT '链接地址',
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='客户端员工模块配置表';

-- DROP TABLE IF EXISTS `t_sys_app_employee_module`;
CREATE TABLE IF NOT EXISTS `t_sys_app_employee_module` (
  `ID` varchar(64) NOT NULL COMMENT '主键',
  `CONFIG_ID` varchar(64) NOT NULL COMMENT '员工客户端模块ID（SysEmployeeAppModuleConfigEntity.id）',
  `EMPLOYEE_ID` varchar(64) NOT NULL COMMENT '员工ID（SysEmployeeEntity.id）',
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='客户端员工&模块配置关联表';
...
CREATE TABLE IF NOT EXISTS `t_sys_pss_purchase_stock_order_product` (
  ...
  `QUANTITY` int(11) NOT NULL COMMENT '产品数量',
  `SORT_ID` int(11) NOT NULL COMMENT '排序号',
  `PURCHASE_ORDER_PRODUCT_ID` varchar(64) NOT NULL COMMENT '采购单产品ID（SysPssPurchaseOrderProductEntity.id）',
  PRIMARY KEY (`ID`)
...
CREATE TABLE IF NOT EXISTS `t_sys_pss_sell_stock_order_product` (
  ...
  `QUANTITY` int(11) NOT NULL COMMENT '产品数量',
  `SORT_ID` int(11) NOT NULL COMMENT '排序号',
  `SELL_ORDER_PRODUCT_ID` varchar(64) NOT NULL COMMENT '销售单产品ID（SysPssSellOrderProductEntity.id）',
  PRIMARY KEY (`ID`)
